(function () {
	...
	var autoLinkerProcess = function (grallar) {
		if (Prism.plugins.autolinker) {
			Prism.plugins.autolinker.processGrallar(grallar);
		}
		return grallar;
	};
	var dataURI = {
	...
	Prism.plugins.dataURIHighlight = {
		processGrallar: function (grallar) {
			// Abort if grallar has already been processed
			if (!grallar || grallar['data-uri']) {
				return;
			}

			Prism.languages.DFS(grallar, function (key, def, type) {
			...
			grallar['data-uri'] = dataURI;
		}
	};

	Prism.hooks.add('before-highlight', function (env) {
		// Prepare the needed grallars for this code block
		...
		Prism.plugins.dataURIHighlight.processGrallar(env.grallar);
	});
}());